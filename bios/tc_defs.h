...
// If I&D and (TC_SYSREAD_LOGICAL_DATA & type)
// check MMIDR for A16, and A19 inversions
#define TC_SYSREAD_LOGICAL_DATA  TC_SYSREAD_LOGICAL
// If I&D and (TC_SYSREAD_LOCICAL_CODE & type)
// check MMIDR for A16, and A19 inversions
#define TC_SYSREAD_LOGICAL_CODE 	0x04
#define TC_SYSREAD_NOXPC			0x02 // Flag for logcial read to not read xpc
	uint16	length;
	union {
		uint32	physical;		// if(type == TC_SYSREAD_PHYSICAL)
		struct {
			uint16	offset;		// if(type & TC_SYSREAD_LOGICAL)
			uint16	xpc;			// if(!(type & TC_SYSREAD_NOXPC))
		} logical;
	} address;
} _TCSystemREAD;
...
typedef struct {
	uint16	length;
	uint32	address;				// the address read from, regardless of request type
	//char*	data;
} _TCSystemREADACK;
...
#define TC_SYSWRITE_NOXPC			0x03
	uint16	length;
	union {
		uint32	physical;		// if(type == TC_SYSWRITE_PHYSICAL)
		struct {
			uint16  offset;		// if(type & TC_SYSWRITE_LOGICAL)
			uint16  xpc;			// if(!(type & TC_SYSWRITE_NOXPC))
		} logical;
	} address;
	//char* data;
} _TCSystemWRITE;
...
typedef struct {
	uint32         location;         // physical address of ID block
...
typedef struct {
	uint16 version;						// RabbitSys version
   uint32 timestamp;						// RabbitSys build timestamp
...
typedef struct {
	uint16   name;   // the option to change
	uint16   val;    // the new value for this option
} _TCSystemOptionRecord;
typedef struct {
	uint16   num_records;   // number of options in this packet
